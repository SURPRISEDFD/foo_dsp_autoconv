...
struct autoconv_preset {
    ...
    bool         resample_enabled = true; // no exact-rate files: use nearest-rate files
                                          // (name AND header must agree on the rate)
                                          // and resample the IR with r8brain - the
                                          // playback audio is never resampled
    ...
    // current 'ACV3' layout plus the legacy 'ACV2' (no resample/FFT flags)
    // and 'ACV1' (carried a filename template) layouts.
